...
parser.add_argument("--model_def", type=str, default=relative_path_prefix +
                    "config/yolov3.cfg", help="path to model definition file")
parser.add_argument("--weights_path", type=str, default=relative_path_prefix +
                    "weights/yolov3.weights", help="path to weights file")
parser.add_argument("--class_path", type=str, default=relative_path_prefix +
                    "data/coco.names", help="path to class label file")
...
class YOLO3_ROS_Node:
    ...
    def callback(self, ros_data):
        ...
        original_img = self.bridge.imgmsg_to_cv2(
            ros_data, desired_encoding="bgr8")
        print(original_img.shape)
        to_square = original_img.shape[1] - original_img.shape[0]
        cv_image = cv2.copyMakeBorder(
            original_img, 0, to_square, 0, 0, cv2.BORDER_CONSTANT)
